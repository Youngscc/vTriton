...
_DEMO_INPUT_DIR = Path(__file__).parents[2] / "data" / "profile_utilization_inputs"
_DEMO_CALIBRATION = _DEMO_INPUT_DIR / "calib_fake_full.json"
...
@pytest.mark.parametrize(
    ...
    [
        ("compute_bound", "Compute Bound", "ComputeBound", "ComputeBound"),
        ("inefficient_compute", "Inefficient Compute", "ComputeBound", "ComputeBound"),
        ("inefficient_mte", "Inefficient MTE", "BandwidthBound", "BandwidthBound"),
        (
        ...
        ("sync_overhead", "Insufficient Parallelism", "SyncOverhead", "ComputeBound"),
    ],
)
def test_demo_profile_utilization_cases_cover_expected_outputs(
    case_name: str,
    operator_diagnosis: str,
    hivm_global_root: str,
    hivm_pipeline_root: str,
):
    """Bundled fake demo inputs should exercise distinct report branches."""
    case_dir = _DEMO_INPUT_DIR / "cases" / case_name
    ...
    assert report.warnings == []
    assert report.diagnosis == operator_diagnosis
    assert report.hivm_bottleneck is not None
    assert report.hivm_bottleneck.warnings == []
